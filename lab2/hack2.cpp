...
//При a=4 и b=1073741824 (например) в 4 строке в цикле while произведение из-за переполнения памяти из 2^32 обернется в 0, что сломает функцию Gcd (цикл while (a*b>0))